...
class Aurora(torch.nn.Module):
    ...
    def __init__(
        ...
        # Adapter
        self.mr_proj_layers = nn.ModuleList([
            nn.ModuleList([nn.Linear(1024, 256), nn.Linear(1024, 256)]),  # 0 for 1/4
            nn.ModuleList([nn.Linear(512, 256), nn.Linear(512, 256)])   # 1 for 1/2
        ])
    ...
    def forward(self, batch: Batch) -> Batch:
        ...
        patch_res = (
            ...
            H // self.encoder.patch_size, # number of patches
        ...
        with torch.autocast(device_type="cuda") if self.autocast else contextlib.nullcontext():
            # Superes: capture multi-resolution latents from backbone
        ...
        # print(f"normal backbone output.shape = {x.shape}")
        ...
        #print("pred.surf_vars['2t'].shape =", pred.surf_vars['2t'].shape)
        

        return pred, multi_preds
    ...
    def load_checkpoint(self, repo: str, name: str, strict: bool = True) -> None:
        ...
        missing_key, unexpect_key = self.load_checkpoint_local(path, strict=strict)
        return missing_key, unexpect_key
    ...
    def load_checkpoint_local(self, path: str, strict: bool = True) -> None:
        ...
        missing_key, unexpect_key = self.load_state_dict(d, strict=strict)
