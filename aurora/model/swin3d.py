...
class Swin3DTransformerBackbone(nn.Module):
    ...
    def __init__(
        ...
        for i_layer in range(self.num_encoder_layers): # 3
    ...
    def forward(
        ...
        z_vq: Optional[torch.Tensor] = None,
        lambda_rd: Optional[torch.Tensor] = None,
        ...
        Args:
            ...
            z_vq (torch.Tensor | None, optional): VQDConv output for RD fusion. Shape `(B, L, D)`.
                Defaults to None. If provided, RD fusion will be performed before final concatenation.
            lambda_rd (torch.Tensor | None, optional): Learnable RD fusion weight parameter.
                Defaults to None. Should be a scalar tensor (will be passed through sigmoid).

        Returns:
            torch.Tensor: Output tokens of shape `(B, L, D*2)` if RD fusion is applied,
                otherwise `(B, L, D*2)` from concatenation.
        """
        ...
        for i, layer in enumerate(self.decoder_layers):
            ...
            # Superes: Collect after each decoder stage
            if(i < 2): # only append the decoder stage 1 output # 2, 1, 0
