...
class Perceiver3DDecoder(nn.Module):
    ...
    def __init__(
        ...
        self.apply(init_weights) # initialization
    ...
    def forward(
        ...
        # Unwrap the latent level dimension. "B (C H W) D -> B (H W) C D"
        x = rearrange(
            x,
            "B (C H W) D -> B (H W) C D",
            C=patch_res[0],
        ...
        # print("H, W =", H, W)
        H,W = patch_res[1] * self.patch_size, patch_res[2] * self.patch_size   # Superes:  Get H, W accoridng to patch_res, * 4 because patch_size=4
        # print("After override, H, W =", H, W)
        #print("x_surf.shape =", x_surf.shape)
        ...
        x_atmos = self.deaggregate_levels(levels_embed, x[..., 1:, :])  # (B, L, C_A, D) # Perceiver
